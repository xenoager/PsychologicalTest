...
import { scoreMBTI } from '../lib/engine.js'
...
export default function Quiz() {
  ...
  function goNext() {
    ...
    const scored = scoreMBTI(quiz || raw, answers)
    ...
    const results = Array.isArray((quiz||raw)?.results)
      ...
      : (typeof (quiz||raw)?.results === 'object' && (quiz||raw)?.results)
        ? Object.entries((quiz||raw).results).map(([id, obj]) => ({ id, ...(obj||{}) }))
        : []
    const result = results.find(r => {
      const pool = [r.id, r.type, r.code, r.mbti, r.slug, r.key, r.value, r.tag, r.kind].filter(Boolean).map(x => String(x).toUpperCase())
      const hit = pool.includes(t)
      const titleHit = String(r?.title || '').toUpperCase().includes(t)
      const listHit = Array.isArray(r?.types) && r.types.map(x => String(x).toUpperCase()).includes(t)
      return hit || titleHit || listHit
    }) || null
    nav(`/${slug}/result/${scored.type}`, { state: { result, scored, quiz: (quiz||raw) } })
  }
  function goPrev() { if (index > 0) setIndex(index - 1) }

  if (loading) return <div className="wrap"><div className="card">로딩 중…</div></div>
  if (error) {
    ...
    return (
      <div className="wrap">
        <div className="card" style={{maxWidth: 880}}>
          ...
          <div style={{color:'#9fb6d1', marginBottom: 8}}>퀴즈 파일을 불러오지 못했습니다.</div>
  ...
  if (!quiz) return <div className="wrap"><div className="card">퀴즈를 찾을 수 없습니다.</div></div>
  ...
  return (
    <div className="wrap">
      <div className="card" style={{width: '100%', maxWidth: 880}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',marginTop:6,fontSize:12,color:'#8aa2c1'}}>
          <div>문항 {Math.min(index+1, total)} / {total}</div>
          <div>{progress}%</div>
        </div>

        <div style={{height: 12}} />
        <div className="qtitle">{index+1}. {q?.title || '문항'}</div>
        ...
        {hasOptions ? (
          ...
          <div className="card" style={{ background:'transparent', border:'1px dashed #263041' }}>
            보기(option)가 정의되어 있지 않아요.
            <small style={{display:'block', marginTop:6, color:'#9fb6d1'}}>
              JSON의 <code>questions[{index}].options</code> 또는 <code>choices/answers</code>를 확인해 주세요.
            </small>
          </div>
        )}

        <div className="toolbar">
          <button className="btn" onClick={goPrev} disabled={index===0}>이전</button>
          <div style={{flex:1}} />
